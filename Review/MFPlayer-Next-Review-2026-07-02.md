...
| NXT-13 | partial | Empty projects gone. God objects + engines remain (by design until deletion). Updated large-file inventory in §5. |
...
3. **Hardware soak of the fixes** (checklist at the end of the Part-4 remediation log), then **engine deletion** (Stage 3 → delete → drop `ShowSessionGate`) — now safe without the escape hatch.
