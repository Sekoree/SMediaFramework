...
The well-scoped correctness/safety findings were fixed in `next/` on branch `next-2`. The large architectural items (NXT-03 *full* off-dispatcher, NXT-06/09/10, and the remainders of NXT-02/04/05/11) are deferred — they are the review's own multi-week "gates" and involve product/design decisions; see *Deferred* below.
...
- **NXT-03 (remainder)** — STOP/LOAD/DISPOSE preemption of an in-flight pre-wait is **done**, and the media open is no longer uninterruptible (the FFmpeg interrupt callback now aborts a blocked open on cancel — see NXT-02). What remains to make a ShowSession STOP actually preempt a slow clip open: thread the cue's cancellation token through the *synchronous* `MediaPlayer.TryOpen` + standby pre-roll (the pre-roll decouples the open from the fire, so this needs care). Beyond that: executing *all* cue work (monitor/fade) off the dispatcher with explicit operation generations. NXT-16 already fixed the query half.
