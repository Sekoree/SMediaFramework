...
The well-scoped correctness/safety findings were fixed in `next/` on branch `next-2`, **plus the two blockers NXT-02 (full atomic + cancellable open) and NXT-03 (full off-dispatcher fire path)**. The remaining large architectural items (NXT-06/09/10 and the remainders of NXT-04/05/11) are deferred — they are the review's own multi-week "gates" and involve product/design decisions or hardware verification; see *Deferred* below.
...
| **NXT-05 (MediaHost)** — 2026-07-01 | The single owning **`MediaHost : IDisposable, IAsyncDisposable`** now exists (`MediaHost.Build`/`Own`): it owns the capability registry, disposes it (releasing the module native runtimes reverse-order) on host dispose, and carries a reference-counted **plugin-lease** surface (`AcquirePluginLease`) with **leaked-lease reporting** on disposal (a warning per owner + an optional escalation hook for debug/test — the seam the NXT-09 plugin host will hang capability/surface/factory leases off so unload can be gated). Both production consumers are consolidated onto it: HaPlay's `MediaRuntime` builds+owns a host and **`MediaRuntime.Shutdown()` disposes it on `desktop.ShutdownRequested`** — closing the concrete gap that the app's *static* registry was **never disposed**, so `Pa_Terminate`/NDI release never ran in the app itself; and the C-ABI `SessionBox` holds a `MediaHost` (destroy/shutdown dispose it). Verified: full managed suite **1365/1365** (Core +8 new `MediaHostTests`), C smoke (empty + real-media) exit 0 with the negative-handle gate intact. **Remaining:** nothing yet *acquires* a plugin lease (inert until the NXT-09 plugin host lands), and the host does not yet own the compositor/control/subtitle/output registries (those are HaPlay-side app state, not framework registries today). | `MediaHost.cs` (new), `MediaRuntime.cs`, `App.axaml.cs`, `NativeApi.cs` | `MediaHostTests` (8: dispose-once/idempotent, async dispose, `Own` ownership, lease released→no leak, outstanding→reported-but-registry-still-released, reporter-throw-safe, post-dispose acquire throws) |
...
| NXT-02/03 (cancellable open) | The FFmpeg open is now **truly cancellable mid-call** — `MediaContainerSharedDemux`'s existing `AVIO` interrupt callback is fed the open's token, so a blocked open (e.g. a hung network connect) aborts on cancel and surfaces as `OperationCanceledException` instead of waiting out the connect timeout. Token threaded `FFmpegModule.OpenAsync → MediaContainerDecoder.Open/OpenUri → SharedDemux.Open`; cleared after open so it can't abort later reads/seeks. | `MediaContainerSharedDemux.cs`, `MediaContainerDecoder.cs`, `FFmpegModule.cs` | `OpenAsync_CancellingABlockedNetworkOpen_AbortsViaInterruptCallback` (opt-in `MFP_RUN_NETWORK_TESTS=1`; verified — hung connect aborted ~88 ms after cancel) |
| NXT-03 (STOP preempts open) | The cue's cancellation token now threads through the (sync) `MediaPlayer.TryOpen` — via a `Cancellation` property on the open builder — and the standby cold-open path, so a ShowSession STOP/LOAD/DISPOSE (which cancels `_activeFireCts`) aborts an in-flight slow clip open instead of waiting it out. | `MediaPlayer.cs`, `MediaPlayerOpenBuilder.cs`, `ClipStandbyEngine.cs` | `Stop_PreemptsABlockedColdOpen_InsteadOfWaitingForIt` |
| **NXT-03 (off-dispatcher)** | Cue fires now run **off** the serial dispatcher: `FireCueAsync`/`GoAsync` acquire a fire-lock (serializing fires the way the app drives GO) and run the cue graph fire off-loop, so a fire's pre-wait and media open no longer park the dispatcher — STOP/seek/load/queries stay responsive throughout. `PlayClipAsync` is split **setup (dispatcher) → open (off) → `CommitClipAsync` (dispatcher)**; a `_showGeneration` bumped on every load + the fire's cancellation token are re-checked at commit, so a fire whose open straddled a reload/STOP discards its now-stale clip instead of corrupting the newer show. `GoAsync` holds the lock across select→fire→advance (no concurrent-GO double-fire). | `ShowSession.cs` (FireCueAsync, FireCoreAsync, GoAsync, PlayClipAsync, CommitClipAsync, LoadDocumentCoreAsync) | `Fire_RunsOffDispatcher_SoASlowOpenDoesNotParkOtherCommands`, `Reload_DuringASlowFireOpen_DiscardsTheStraddlingClip` (+ all 61 Session tests, HaPlay 472, SessionSmoke real playback) |
| NXT-06 (cue video fan-out) | Audit of the `HAPLAY_USE_SHOWSESSION` cutover path found cue **video-output realization already wired** (a stale "later slice" comment had hidden it): `ReloadCueShowSession` acquires the real OutputManagement NDI/SDL/local outputs per composition→line binding and feeds them through the session's composition-id video factory. Closed the test gap with an end-to-end headless proof (mapped cue → ShowSession factory → fan-out to a host-acquired output) and corrected the stale comment. | `MainViewModel.cs` (comment), `CueVideoOutputFanoutTests.cs` (new) | `CueVideo_FansOutToHostAcquiredOutputs_ViaTheCompositionIdFactory` (+ HaPlay 473) |
| NXT-13 (partial) | Removed the two empty leftover projects (`S.Media.Images.Skia`, `S.Media.Encode.FFmpeg`) from the solution and deleted their shells + dangling `InternalsVisibleTo` grants. (The larger god-object split / duplicate-engine retirement remains with the cutover work.) | `MFPlayer.Next.sln`, `S.Media.Decode.FFmpeg.csproj`, `S.Media.FFmpeg.Common.csproj` | full build still 0 errors |
| NXT-02 | **Atomic async media open** — `IMediaRegistry.OpenAsync(MediaOpenRequest) → MediaOpenResult`. A correlated A/V file now opens through **one shared demux** (FFmpeg's `MediaContainerDecoder`) — one open/probe, one buffering/seek state — instead of two independent contexts. The result OWNS the asset (single dispose tears down both tracks), exposes duration / live / seekable + track enumeration + attached-picture, preserves the provider's real open error, and is cancellable (worker-thread open, token honoured). `MediaPlayer.TryOpen` consumes it, so **all** file playback (ShowSession + the builder paths) uses one demux; NDI shares one receiver. Providers that don't override get a default bridge to the per-kind opens (so fakes/ABI are unchanged). Verified end-to-end via SessionSmoke real playback. | `MediaOpenResult.cs` (new), `IMediaDecoderProvider.cs`, `IMediaRegistry.cs`, `MediaRegistry.cs`, `FFmpegModule.cs`, `NDIModule.cs`, `MediaPlayer.cs` | `OpenAsync_AtomicResult_OwnsAsset_DisposedExactlyOnce`, `OpenAsync_HonoursCancellation`, `OpenAsync_DefaultBridge_*`; SessionSmoke |
...
- **NXT-02 (remainder)** — the atomic async single-demux open **and true mid-native-call cancellation** are done: the FFmpeg `AVIO` interrupt callback is now fed the open's token, so a blocked open aborts on cancel (verified — a hung network connect aborts ~immediately on cancel via `MediaContainerSharedDemux`). What remains: a separate `PrepareAsync`/content-cache stage for network sources (the YouTube gate), and threading the cue token through the *synchronous* `MediaPlayer.TryOpen` + standby pre-roll so a ShowSession STOP preempts a slow clip open (the NXT-03 application wiring below).
...
- **NXT-14 (remainder)** — the broader adversarial/sync/perf/cutover test build-out beyond the targeted regressions added so far (much of it depends on the deferred gates' behavior existing first). *Progress (2026-07-01):* the end-of-clip state machine — previously covered only for the natural-end **fade** path — now has explicit **loop / freeze / plain-stop** coverage (`NaturalEnd_Loop_KeepsRunningPastOutPoint_InsteadOfReleasing`, `NaturalEnd_Freeze_HoldsClipInsteadOfReleasing`, `NaturalEnd_PlainStop_ReleasesClipAtOutPoint` in `ShowSessionTests`, using an `EndOffset`-trimmed synthetic clip so each behaviour fires fast and is distinguished by the transport snapshot: loop → still running past the out-point; freeze → paused but still attached; stop → detached/zero-duration). Session tests 67 → 70. *Flip-confidence pass (2026-07-01):* added coverage of the
  newest/least-soaked seams — the multi-composition fan-out (`ClipWithMultiplePlacements_FansVideoToEveryComposition`
  masters every canvas; `..._TearsDownEveryLayer_OnStop` proves no layer leaks on stop; `MultiPlacement_LivePlacement
  Edit_TargetsTheAddressedLayer` proves a live edit resolves the addressed (composition, layer) and rejects a
  no-match key), the calibration-grid injection (`TestPattern_AddsReplacesAndClearsATopLayer` — add/replace-reuse/
  clear + unknown-composition rejection), the nested-group collapse (`NestedGroups_CollapseIntoOutermostGroupId`),
  and the validator's new fan-out reference check (`ExtraPlacements` → unknown composition). Also added **gate
  telemetry**: `TryWireShowSessionCueTransport` now logs one startup line naming the active cue playback path
  (`ShowSession (convergence)` vs `legacy CuePlaybackEngine`) on BOTH branches, so a soak log is unambiguous about
  which engine ran. Session 70 → 81.
