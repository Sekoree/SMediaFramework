    ...
    static ClassicTheme()
    {
        ...
        Control.LoadedEvent.AddClassHandler<Window>((window, _) =>
        {
            if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
            {
                ...
                // On macOS None decorations still allow dragging/resizing.
                window.WindowDecorations = global::Avalonia.Controls.WindowDecorations.None;
            }
            else
            {
                // With Full on Windows 11 we get rounded borders and shadow (bad), but with None,
                // dragging and resizing don't work.
                window.WindowDecorations = global::Avalonia.Controls.WindowDecorations.Full;
            }
        });
