...
| **Plugins** | Two tiers, one dynamic mechanism: **(A)** AOT-pure typed registration for build-time modules, **(B)** a **native C-ABI plugin** contract for third parties — any NativeAOT/C/C++/Rust `.so`/`.dll` exposing the plugin C API. No managed reflection/ALC loading. See [05](05-Plugin-Model.md). |
...
5. **[05 — Plugin Model](05-Plugin-Model.md)** — the typed registry + the native C-ABI plugin contract.
...
```
                         ┌───────────────────────────────────────────────┐
   Native wrappers       │  PALib  MALib  PMLib  NDILib  OSCLib  (JackLib) │   ← KEEP as-is (P/Invoke + OSC)
   (+ NuGet: FFmpeg.AutoGen, SDL3-CS, Silk.NET.OpenGL, SkiaSharp)         │
                         └───────────────────────────────────────────────┘
                                              ▲
   ┌──────────────────────────────────────────────────────────────────────────────┐
   │ S.Media.Core      pure primitives + contracts + registry (NO backend deps)     │  Tier 1
   ├──────────────────────────────────────────────────────────────────────────────┤
   ...
   │   Decode.FFmpeg · Encode.FFmpeg · Audio.PortAudio · Audio.MiniAudio            │
   ...
   ├──────────────────────────────────────────────────────────────────────────────┤
   │ S.Control   MIDI/OSC/Mond + device-profile registry                           │  Tier 6
   ├──────────────────────────────────────────────────────────────────────────────┤
   │ S.Media.Abi   native C-ABI plugin host   ·   S.Media.Interop   outbound C ABI  │  Tier 7
   └──────────────────────────────────────────────────────────────────────────────┘
                                              ▲
...
- Perfect A/V sync across **multiple inputs** (file + live: NDI, mic/line capture) and **multiple outputs**.
