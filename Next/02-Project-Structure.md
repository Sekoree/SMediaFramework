...
**New in Core:**
- The **registry contracts**: `IMediaModule`, `IMediaRegistry`, `IMediaRegistryBuilder`, capability
  provider interfaces (decoder/backend/presenter/image/subtitle/layer-surface factories). See [05](05-Plugin-Model.md).
...
`SessionClock` (master) + `SourceTimeline` (per-source offset + rebase policy) — see [03](03-AV-Sync-Clocks-Routing.md).
...
they don't re-implement uploads. Leave a thin internal seam (`IGpuDevice`) so a future
Vulkan/Metal backend is possible — but ship GL only.
...
into the GL canvas (the "3D object layer" idea). See [04](04-Compositor-Warp-GPU.md) and [05](05-Plugin-Model.md).

---

## Tier 3b — backend modules (capabilities; each registers itself)

Each is an `IMediaModule`. None reference each other. Presence in the registry = capability available.

| Project | Deps | Provides | Salvage from |
|---|---|---|---|
| `S.Media.Decode.FFmpeg` | Core (+FFmpeg.AutoGen) | file/stream `IVideoSource`+`IAudioSource`, hw decode (D3D11VA/VAAPI/…), `IVideoCpuFrameConverter` (swscale), `IDeinterlacer` (yadif), audio resampler, **capture** sources (v4l2/dshow), track enumeration | `S.Media.FFmpeg` (8.5k) |
| `S.Media.Encode.FFmpeg` | Core, Decode.FFmpeg | muxers/encoders for record/stream out | `S.Media.FFmpeg.Encode` |
...
| `S.Media.Subtitles` | Core (+libass / FFmpeg sub decode) | timed subtitle layer source (SRT/VTT/ASS/PGS) | **new** — see [04](04-Compositor-Warp-GPU.md) §subtitles |

> JACK can ship later as `S.Media.Audio.Jack` (→ Core, Routing, JackLib) — same `IAudioBackend` seam.

---

## Tier 4 — `S.Media.Players`  *(→ Core, Time, Routing)*
...
## Tier 5 — `S.Media.Session`  *(→ Core, Time, Routing, Players, Compositor, registry)*
...
### `S.Media.Abi`  *(→ Core, Session)*
**Job:** the **inbound** native plugin host. Defines the C plugin ABI (`include/mfp_plugin.h`),
`dlopen`s plugins, and adapts their vtables into `IAudioBackend`/`IVideoSource`/`IVideoOutput`/
`IVideoCompositorLayerSurface`, registering them in the registry. See [05](05-Plugin-Model.md).
...
+ **9** backend modules + **2** interop/abi + **1** control = **~19**, plus 6 native wrappers and the
