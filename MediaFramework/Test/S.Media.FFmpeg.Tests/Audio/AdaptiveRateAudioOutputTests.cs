...
public sealed class AdaptiveRateAudioOutputTests
{
    ...
    {
        var inner = new RecordingOutput(Stereo48K);
        using var adaptive = new AdaptiveRateAudioOutput(inner, () => 0.0, maxRateDeltaHz: 3);
    ...
    {
        var inner = new RecordingOutput(Stereo48K);
        using var adaptive = new AdaptiveRateAudioOutput(inner, () => -400.0, maxRateDeltaHz: 12);
        var chunk = new float[480 * 2];
        for (var i = 0; i < chunk.Length; i++)
    ...
    {
        var inner = new RecordingOutput(Stereo48K);
        using var adaptive = new AdaptiveRateAudioOutput(inner, () => 0.0);
        using var cts = new CancellationTokenSource();
        Assert.True(((IClockedOutput)adaptive).WaitForCapacity(256, cts.Token));
        Assert.Equal(1, inner.WaitCalls);
    }

    private sealed class RecordingOutput(AudioFormat format) : IAudioOutput, IClockedOutput
    {
