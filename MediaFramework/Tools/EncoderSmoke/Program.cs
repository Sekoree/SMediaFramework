...
//   dotnet run --project MediaFramework/Tools/EncoderSmoke -- --push srt://host:9000 --keepalive-only
//       → verify the session's between-cues black/silence signal and reconnect path
...
for (var i = 0; i < args.Length; i++)
{
    switch (args[i])
    {
        ...
        case "--video-bitrate": videoBitrate = long.Parse(args[++i]); break;
        case "--keepalive-only": keepaliveOnly = true; break;
        default:
...
if (filePath is null && pushUrl is null && servePort is null)
{
    Console.Error.WriteLine(
        "usage: EncoderSmoke (--file <path> | --push <url> | --serve <port>)… " +
        "[--seconds N] [--width N --height N --fps N --video-bitrate BPS] [--keepalive-only]");
    return 2;
}
if (keepaliveOnly && pushUrl is null && servePort is null)
{
    Console.Error.WriteLine("--keepalive-only requires --push and/or --serve.");
    return 2;
}
...
if (pushUrl is not null || servePort is not null)
{
    ...
    if (!keepaliveOnly)
    {
        // Declare both routes. The activity-aware wrappers keep sending filler until the first generated
        // sample actually arrives, then hand each track back if the generator later falls silent.
        liveSession.SetPlaybackActive(videoActive: true, audioActive: true);
    }
...
if (keepaliveOnly)
{
    for (var second = 0; second < seconds; second++)
    {
        ...
        Console.Write($"\r{second + 1}/{seconds}s ");
    }
}
else
{
