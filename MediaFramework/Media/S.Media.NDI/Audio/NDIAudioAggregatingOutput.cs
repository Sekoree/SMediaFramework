...
/// <summary>
/// Buffers packed-float audio and forwards to an inner output (for example <see cref="NDIAudioOutput"/>)
...
/// <remarks>
/// <para>
/// <see cref="Dispose"/> may flush a partial final block to the inner output; <strong>Debug</strong> builds log a failed final
/// <see cref="IAudioOutput.Submit"/> via <see cref="MediaDiagnostics.LogError"/> while <strong>Release</strong> continues.
/// </para>
/// </remarks>
public sealed class NDIAudioAggregatingOutput : IAudioOutput, IAudioOutputChannelCapabilities, IDisposable
{
    ...
    public NDIAudioAggregatingOutput(IAudioOutput inner, int targetSamplesPerChannel)
    {
    ...
    public void Dispose()
    {
        ...
        {
            MediaDiagnostics.SwallowDisposeErrors(() => _inner.Submit(_buffer.AsSpan(0, _filledFloats)), "NDIAudioAggregatingOutput.Dispose: final Submit");
        }
        _filledFloats = 0;
    }
}
