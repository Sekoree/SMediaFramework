...
/// </para>
/// <para>
/// Implements <see cref="IFlushableOutput"/> so the <c>AudioRouter</c> pause/seek drops buffered
...
public class ResamplingAudioOutput : IAudioOutput, IAudioOutputChannelCapabilities, IFlushableOutput, IDisposable
{
    ...
    /// <param name="routerFormat">Format the <c>AudioRouter</c> uses for this route (typically the decoder rate).</param>
    ...
    public void Flush()
    {
        ...
        // playback sessions. A torn-down session's leftover Flush would otherwise abort the next session's
        // live stream. The owning session disposes this wrapper as part of its teardown.
