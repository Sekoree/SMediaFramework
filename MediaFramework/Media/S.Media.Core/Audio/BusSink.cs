...
/// <summary>
/// Sub-mix bus: implements both <see cref="IAudioSink"/> (routes can target it) and
...
/// <remarks>
/// <para>
/// Wiring pattern: register the bus on the router as <strong>both</strong> a sink (via
/// <see cref="AudioRouter.AddSink"/>) and a source (via <see cref="AudioRouter.AddSource"/>).
/// Per chunk, the router pumps mixed audio into the bus (sink side) and reads it back on the
/// source side. The bus introduces approximately one router chunk of latency through the feedback
/// path — the SPSC ring decouples the per-sink pump thread (Submit) from the run loop thread
...
/// and run-loop threads doesn't underflow. Override via <see cref="BusSink(AudioFormat, TimeSpan?)"/>
...
public sealed class BusSink : IAudioSink, IAudioSinkChannelCapabilities, IAudioSource
{
    ...
    public BusSink(AudioFormat format, TimeSpan? maxBufferedDuration = null)
    {
