...
/// release keyed mutex key <c>0</c> for the NV12 texture before the <see cref="VideoFrame"/> is handed to outputs;
...
/// </remarks>
public sealed class Win32SharedNv12Backing : VideoFrameHardwareBacking
{
    ...
    public void AddReference()
    {
        while (true)
        {
            ...
            if (n <= 0)
                throw new ObjectDisposedException(nameof(Win32SharedNv12Backing));
    ...
    {
        if (!OperatingSystem.IsWindows())
            throw new PlatformNotSupportedException(VideoFrame.CreateNv12Win32SharedOnlyWindows);
