...
/// Each input is an <see cref="IVideoSink"/> slot the upstream router targets; the single output
/// is an <see cref="IVideoSource"/> a downstream player or router pulls from.
/// </summary>
/// <remarks>
/// <para>
/// Same source/sink-duality pattern as <see cref="Audio.BusSink"/>, but **per slot**: each slot
/// holds only the most-recent submitted frame (replace-on-submit). When the downstream consumer
/// calls <see cref="TryReadNextFrame"/>, the sink snapshots every slot's current frame, builds a
...
/// This matches <see cref="VideoSinkPump"/>'s drop-oldest behavior under pressure but at the slot
...
/// </para>
/// <para>
/// <strong>Threading</strong>: <see cref="IVideoSink.Submit"/> on a slot may run on any thread
...
/// </para>
/// </remarks>
public sealed class CompositorVideoSink : IVideoSource, IDisposable
{
    ...
    /// <param name="compositor">The compositor that does the actual blending. Lifecycle: owned by the sink when <paramref name="disposeCompositorOnDispose"/> is <c>true</c>.</param>
    /// <param name="disposeCompositorOnDispose">When <c>true</c> (default), <see cref="Dispose"/> also disposes the compositor.</param>
    public CompositorVideoSink(VideoFormat output, IVideoCompositor compositor, bool disposeCompositorOnDispose = true)
    {
    ...
    /// <summary>
    /// Adds an input slot. The returned <see cref="Slot"/> exposes the <see cref="IVideoSink"/>
    /// upstream code should target (<see cref="Slot.Sink"/>) plus mutable per-slot
    ...
    /// <param name="acceptedFormats">Override the slot's <see cref="IVideoSink.AcceptedPixelFormats"/>. Defaults to the compositor's <see cref="IVideoCompositor.AcceptedLayerPixelFormats"/>.</param>
    ...
    public void SelectOutputFormat(PixelFormat format)
    {
        if (format != _output.PixelFormat)
            throw new InvalidOperationException(
                $"CompositorVideoSink only delivers {_output.PixelFormat}; consumer requested {format}.");
    }
    ...
    /// <summary>One input slot — combines an <see cref="IVideoSink"/> target with mutable composite parameters.</summary>
    public sealed class Slot
    {
        private readonly Lock _gate = new();
        private readonly SlotSink _sink;
        ...
        {
            Id = id;
            _sink = new SlotSink(this, accepted);
        }

        /// <summary>Stable id for diagnostics.</summary>
        public string Id { get; }

        /// <summary>The <see cref="IVideoSink"/> upstream code targets.</summary>
        public IVideoSink Sink => _sink;
        ...
        internal void SubmitFromSink(VideoFrame frame)
        {
        ...
        private sealed class SlotSink(Slot owner, IReadOnlyList<PixelFormat> accepted) : IVideoSink
        {
