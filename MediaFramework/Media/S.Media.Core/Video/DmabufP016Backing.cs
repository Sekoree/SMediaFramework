...
/// </summary>
public sealed class DmabufP016Backing : VideoFrameHardwareBacking
{
    private int _yFd;
    private int _uvFd;
    private int _refCount = 1;

    public DmabufP016Backing(
        int yPlaneDupFd,
    ...
    public void AddReference()
    {
        while (true)
        {
            ...
            if (n <= 0)
                throw new ObjectDisposedException(nameof(DmabufP016Backing));
    ...
    {
        if (!OperatingSystem.IsLinux())
            throw new PlatformNotSupportedException(VideoFrame.CreateP016DmabufOnlyLinux);
