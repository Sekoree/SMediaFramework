...
public sealed partial class AudioRouter : IDisposable
{
    ...
    public string AddSource(IAudioSource source, string? id = null, bool? autoResample = null)
    {
        ...
        if (source.Format.SampleRate != _sampleRate)
        {
            ...
            if (ResamplerFactory is not { } factory)
                throw new InvalidOperationException(
                    "AudioRouter.AddSource: autoResample requested but no ResamplerFactory is set " +
                    "(wire it from the media registry, e.g. registry.CreateResampler).");
    ...
    /// members (e.g. prior <c>AudioClipVoice</c> instances in the same pad group).
    /// </summary>
    ...
    {
        if (source is IStoppableSource voice)
            voice.Stop();
    }
