...
/// <summary>
/// <see cref="IVideoSink"/> backed by an SDL3 window + renderer. Two
/// dispatch modes:
/// <list type="bullet">
///   <item><b>Auto-thread</b> (default): the sink owns its render thread.
...
/// UYVY, and YUY2 — picking any of those means the sink uploads the source's
/// planes directly (no CPU conversion).
/// </para>
/// <para>
/// Window lifecycle: the sink polls SDL events each render cycle (or each
/// <see cref="Pump"/> call in manual mode) so the window stays responsive.
/// Closing the window does not stop the sink; <see cref="CloseRequested"/>
/// fires and the host decides when to <see cref="Dispose"/>.
/// </para>
/// </remarks>
public sealed unsafe class SDL3VideoSink : IVideoSink, IDisposable
{
    ...
    public VideoFormat Format
    {
        ...
        {
            if (!_configured)
                throw new InvalidOperationException("SDL3VideoSink.Configure has not been called yet");
            return _format;
        }
    }
    ...
    /// <summary>True when the sink owns its render thread (auto mode).</summary>
    ...
    /// the window to close). The sink does NOT auto-dispose — host code
    ...
    /// When <c>true</c> (default), the sink starts a dedicated render thread
    ...
    /// </param>
    public SDL3VideoSink(string title = "SDL3 Video", int initialWidth = 1280, int initialHeight = 720,
    ...
    public void Configure(VideoFormat format)
    {
        ...
        if (_configured)
            throw new InvalidOperationException("SDL3VideoSink already configured; create a new sink to switch format");
        if (Array.IndexOf(AcceptedFormats, format.PixelFormat) < 0)
            throw new NotSupportedException(
                $"SDL3VideoSink does not accept pixel format {format.PixelFormat}; supported: {string.Join(", ", AcceptedFormats)}");
    ...
    public void Submit(VideoFrame frame)
    {
        ...
        {
            frame.Dispose();
            throw new InvalidOperationException("SDL3VideoSink.Submit called before Configure");
        }
    ...
    {
        if (_ownsThread)
            throw new InvalidOperationException("SDL3VideoSink.Pump called on an auto-thread sink — use ownsThread:false");
        if (_disposed) return;
