...
/// <summary>
/// Wraps an <see cref="IAudioOutput"/> (and optional <see cref="IClockedOutput"/>) and applies a
/// small libav <c>swresample</c> rate tweak so a output that cannot keep up with the router
/// (sustained <see cref="AudioRouter.PumpPressure"/> drops on that output id) receives slightly
/// fewer samples per chunk, easing queue growth without retuning the global playback clock.
/// </summary>
/// <remarks>
/// <para>
/// Negative <see cref="PumpPressurePlaybackHintMonitor.HintPpmBias"/> means the output is slow
/// relative to production; this output maps that to a slightly lower output sample rate
/// (e.g. 48&nbsp;000&nbsp;Hz → 47&nbsp;997&nbsp;Hz) so libav’s resampler shaves a few
/// frames over time. Positive bias does the opposite when a output is starved (rare with bounded
...
/// </para>
/// <para>
/// When you remove the output from the <see cref="AudioRouter"/>, dispose this
/// wrapper (after <see cref="AudioRouter.RemoveOutput"/>) so the pressure monitor unsubscribes.
/// </para>
/// <para>
/// This wrapper applies <see cref="PumpPressurePlaybackHintMonitor"/> hints per output only. Coordinated multi-output master-clock ppm and explicit
/// drop/repeat scheduling remain a host concern; composite master selection lives on <see cref="S.Media.Core.Clock.MediaClock"/> via
/// <see cref="S.Media.Core.Clock.MediaClockExtensions.SetMasterChain"/>, and session-level coordination is described on <see cref="S.Media.Core.Playback.AvPlaybackCoordinator"/>.
/// </para>
/// <para>
...
public sealed class AdaptiveRateAudioOutput : IAudioOutput, IAudioOutputChannelCapabilities, IClockedOutput, IAdaptiveRateWrappedOutput, IDisposable
{
    ...
    /// <summary>
    /// Adapts using <see cref="AudioRouter.PumpPressure"/> for a single <paramref name="outputId"/>.
    /// </summary>
    public AdaptiveRateAudioOutput(
        IAudioOutput inner,
        AudioRouter router,
        string outputId,
        double maxAbsPpm = 40,
        ...
        int maxRateDeltaHz = 3)
    {
        ...
        _hintMonitor = new PumpPressurePlaybackHintMonitor(router, outputId, maxAbsPpm, ppmPerDropPerSecond);
        _getPpmBias = null;
        _effectiveOutRate = 0;
    ...
    public void Dispose()
    {
        if (_disposed)
            return;
        _disposed = true;
        MediaDiagnostics.SwallowDisposeErrors(() => _hintMonitor?.Dispose(), "AdaptiveRateAudioOutput.Dispose: PumpPressurePlaybackHintMonitor");
        lock (_resampleGate)
        {
