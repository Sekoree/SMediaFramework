...
/// optional <see cref="AudioRouter"/> + <see cref="MediaClock"/>, optional <see cref="VideoRouter"/>, with a fixed <see cref="Dispose"/>
...
public sealed class MediaContainerPlaybackBundle : IDisposable
{
        ...
        MediaContainerPlaybackBundleOwnedParts ownedParts)
    {
        ...
        if (_owned.HasFlag(MediaContainerPlaybackBundleOwnedParts.AudioRouter) && AudioRouter is null)
            throw new ArgumentException("AudioRouter ownership requested but audioRouter is null.", nameof(audioRouter));

        Session = MediaContainerSession.Create(Decoder, Video, Clock, AudioRouter, AudioClock, AudioSourceId);
    }
    ...
    public void Dispose()
    {
        ...
        if (_owned.HasFlag(MediaContainerPlaybackBundleOwnedParts.AudioRouter))
        {
            if (AudioRouter is not null)
                TryDisposeOwned(() => AudioRouter.Dispose(), "MediaContainerPlaybackBundle.Dispose: AudioRouter");
            if (AudioClock is not null)
                TryDisposeOwned(() => AudioClock.Dispose(), "MediaContainerPlaybackBundle.Dispose: AudioClock");
        }
