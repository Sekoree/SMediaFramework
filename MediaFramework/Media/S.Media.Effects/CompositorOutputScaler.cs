...
public sealed class CompositorOutputScaler : IDisposable
{
    ...
    public CompositorOutputScaler(VideoFormat output, LayerConfig? config = null)
    {
        _output = output;
    ...
    public bool TryComposite(VideoFrame input, out VideoFrame? output)
    {
        ...
        if (!CompositorBgraHelper.TryToBgra(input, ref _toBgra, out var layer) || layer is null)
            return false;
