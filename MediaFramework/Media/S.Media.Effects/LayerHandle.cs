...
public sealed class LayerHandle
{
    ...
    public LayerConfig CurrentConfig => _config;

    public void SetConfig(LayerConfig config) => _config = config;
    ...
    public void ClearTransitions() => _transitions.Clear();
    ...
    internal bool TryPullFrame(VideoFormat canvasFormat, out VideoFrame? frame, out bool converted)
    {
        ...
        var resolved = LayerConfigResolver.ResolveAt(_config, _transitions, timeline);
        ...
        Slot.Transform = LayerConfigResolver.ResolveTransform(_config, _transitions, timeline, src.Format, canvasFormat);
        ...
        Slot.Output.Configure(frame.Format);
