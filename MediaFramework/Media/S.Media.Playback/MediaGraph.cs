...
public sealed class MediaGraphFileBuilder(string filePath)
{
    ...
    /// <summary>Controls whether the graph owns the decoder created from <see cref="FilePath"/>.</summary>
    public MediaGraphFileBuilder WithDecoderOwnership(MediaPlayerDecoderOwnership ownership)
    {
        _decoderOwnership = ownership;
        return this;
    }

    /// <summary>Builds the graph and returns false instead of throwing on open/wiring failure.</summary>
    public bool TryBuild([NotNullWhen(true)] out MediaGraph? graph, out string? error)
    {
        var builder = MediaPlayer.OpenFile(FilePath)
            .WithOptions(_options)
            .WithDecoderOwnership(_decoderOwnership);

        if (_videoOutput is not null)
            builder.WithVideoLead(_videoOutput, _disposeVideoOutput);

        if (!builder.TryBuildSession(out var session, out error))
        {
            graph = null;
            return false;
        }

        graph = new MediaGraph(session, MediaGraphTopology.FilePlayback, "file playback");
        return true;
    }
    ...
    public MediaGraph Build()
    {
        if (!TryBuild(out var graph, out var error))
            throw new InvalidOperationException(error ?? "Media graph build failed.");
        return graph;
    }

    /// <summary>Async <see cref="Build"/> for UI hosts.</summary>
    public Task<MediaGraph> OpenAsync(CancellationToken cancellationToken = default) =>
        Task.Run(() =>
        {
