...
// UYVY packed in an RGBA8 texture at half the luma width (each texel: U, Y0, V, Y1).
// Use nearest sampling so macropixels are not blended.
...
void main()
{
    float x = v_uv.x * frameWidth;
    float hx = floor(x);
    float pairIdx = floor(hx * 0.5);
    float s = (pairIdx + 0.5) / halfTexWidth;
    vec4 uyvy = texture(packedTex, vec2(s, v_uv.y));
    float Y = mod(hx, 2.0) < 0.5 ? uyvy.g : uyvy.a;
    float U = uyvy.r;
    float V = uyvy.b;
