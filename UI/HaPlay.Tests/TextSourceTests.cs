...
/// <summary>SESSION-02: the <c>text:</c> source, now the first-party <c>S.Media.Source.Text</c> module, still
/// drives the HaPlay cue path. Covers the URI round-trip, render + duration-bounding, and the mapper emitting a
/// <c>text:</c> binding via the HaPlay → framework spec mapping. Visual correctness is verified on hardware; these
/// pin the headless open/bind path.</summary>
public class TextSourceTests
{
    ...
    public void TextSourceUri_RoundTrips_RenderSpecAndDuration()
    {
        ...
        var uri = TextSourceUri.Encode(item.ToSpec(durationMs: 1500));
    ...
    {
        var uri = TextSourceUri.Encode(new TextPlaylistItem { Text = "Hi", CanvasWidth = 64, CanvasHeight = 48 }.ToSpec(200));
        ...
        Assert.Equal(TimeSpan.FromMilliseconds(200), src.Duration); // reported → the session's monitor ends at duration
    ...
    public void TextDecoderProvider_ZeroDuration_HoldsUnbounded()
    {
        using var src = (TextHeldVideoSource)new TextDecoderProvider()
            .OpenVideo(TextSourceUri.Encode(new TextPlaylistItem { Text = "Hold", CanvasWidth = 32, CanvasHeight = 32 }.ToSpec(0)), null);
    ...
    {
        // Live text update: a playing text cue's frame can be swapped in place (IReplaceableFrameSource) when its
        // text/style is edited, without re-firing. A size change proves the new frame took effect.
        using var src = (TextHeldVideoSource)new TextDecoderProvider()
            .OpenVideo(TextSourceUri.Encode(new TextPlaylistItem { Text = "A", CanvasWidth = 64, CanvasHeight = 48 }.ToSpec(0)), null);
        ...
        var replacement = TextFrameRenderer.Render(
            new TextPlaylistItem { Text = "B", CanvasWidth = 100, CanvasHeight = 40 }.ToSpec(), new Rational(30, 1));
    ...
    public void TextFrameRenderer_RendersAtTheCanvasSize()
    {
        ...
        var frame = TextFrameRenderer.Render(
            new TextPlaylistItem { Text = "Hi", CanvasWidth = 320, CanvasHeight = 180, FontSizePx = 96 }.ToSpec(),
            new Rational(30, 1));
