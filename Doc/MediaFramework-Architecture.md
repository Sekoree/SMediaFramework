...
Types covered here:
- [`AudioRouter`](#audiorouter) — graph routing & per-sink pumps
...
When more than one output is attached, only the sink wired via `SlaveTo` paces the router via its `IClockedSink.WaitForCapacity`. All other sinks (a second PortAudio device, an NDI sender, …) run off their own physical clock — PortAudio's hardware crystal, NDI's internal pace, etc.

These secondary clocks drift relative to the master at typical hardware tolerances around ±50 ppm. Over minutes-to-hours this accumulates: the non-slaved sink's pump either runs ahead (fills up, then drops oldest chunks once `pumpCapacityChunks` is exceeded) or falls behind (empties out, the sink starves and the application hears clicks / sees frame stalls).

Recommended host-side mitigations:

1. **Subscribe to `AudioRouter.PumpPressure`** and react when `Dropped` increments on a specific sink id — back off the route, lower its gain, or fan out a "we're dropping audio" diagnostic.
2. **Wrap the slow sink in `AdaptiveRateAudioSink`** (FFmpeg). The wrapper runs a tiny per-sink `swresample` driven by `PumpPressurePlaybackHintMonitor` filtered to that sink id. It applies a few-ppm rate tweak only on the path into that sink, leaving the router's nominal graph rate unchanged.
3. **`GetAggregatePumpStats`** sums every per-sink `SinkPumpStats` into a single aggregate suitable for HUD overlays or periodic logging. It does *not* synthesize a global master clock or coordinated drop policy — those are intentionally not provided. Hosts that need coordinated drop/repeat across multiple sinks must implement it on top of these primitives.

### What's not implemented

The router deliberately omits:

- A single coordinated **master** clock ppm policy (every sink paced to the slowest one). The current model is "one slaved sink + everyone else drifts" + per-sink adaptive resampling.
- Synchronized **drop / repeat** across sinks. Each sink's pump drops oldest independently. If you need lock-step gap insertion across two sinks, you'll need to build it on top of `PumpPressure` events.
...
1. Build GL / NDI / PortAudio sinks (each lives on whatever assembly it belongs to — `S.Media.SDL3`, `S.Media.NDI`, `S.Media.PortAudio`).
...
If you only need a single-file playback path with router + optional sinks (no SDL / NDI / PortAudio), prefer `S.Media.Playback.MediaPlayer.TryOpen` — it's the library-side entry that returns a configured player with no host-platform assemblies referenced.
