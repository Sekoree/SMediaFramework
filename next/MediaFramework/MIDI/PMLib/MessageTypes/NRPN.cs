...
/// <summary>
/// Composite MIDI message representing a Non-Registered Parameter Number change.
/// Encodes a 14-bit parameter number (CC 99/98) and a 14-bit value (CC 6/38).
/// </summary>
/// <remarks>
/// <b>Wire format:</b> <see cref="WriteTo"/> sends four consecutive CC messages:
/// <list type="number">
///   <item><description>CC 99 — NRPN parameter MSB</description></item>
///   <item><description>CC 98 — NRPN parameter LSB</description></item>
...
/// assemble individual CC messages into complete <see cref="NRPN"/> instances.
/// </remarks>
public readonly struct NRPN : IMIDIMessage
{
    /// <summary>MIDI channel, 0–15.</summary>
    public byte Channel { get; }

    /// <summary>
    /// 14-bit parameter number (0–16383), assembled from CC 99 (MSB) and CC 98 (LSB).
    /// </summary>
    ...
    public MIDIMessageType MessageType => MIDIMessageType.NRPN;

    /// <summary>
    /// Constructs an NRPN message from a 14-bit parameter number and 14-bit value.
    /// </summary>
    public NRPN(byte channel, ushort parameter, ushort value)
    {
        Channel   = channel;
    ...
    /// <summary>
    /// Constructs an NRPN message from individual MSB/LSB bytes.
    /// </summary>
    public NRPN(byte channel, byte paramMsb, byte paramLsb, byte dataMsb, byte dataLsb)
    {
        Channel   = channel;
    ...
    public PmError WriteTo(nint stream, int timestamp)
    {
        ...
        var err = Native.Pm_WriteShort(stream, timestamp,
            PmEvent.CreateMessage(status, 99, ParameterMsb));
        if (err != PmError.NoError) return err;

        err = Native.Pm_WriteShort(stream, timestamp,
            PmEvent.CreateMessage(status, 98, ParameterLsb));
        if (err != PmError.NoError) return err;
