...
public sealed class VideoCompositor : IVideoSource, IDisposable
{
    ...
    private VideoCompositor(VideoFormat output, IVideoCompositor compositor, Func<IVideoCpuFrameConverter>? cpuConverterFactory)
    {
        _output = output;
    ...
    /// <summary>
    /// Supplies the master timeline position for clock-driven advance (set by the player, which owns the
    /// clock from <c>S.Media.Time</c>). When null, the compositor runs read-paced (one inner frame per read).
    /// A delegate keeps the compositor free of a direct <c>S.Media.Time</c> dependency (arch boundary).
    /// </summary>
    public Func<TimeSpan>? MasterTimeProvider { get; set; }

    /// <summary>Registry-wired CPU converter factory for non-BGRA layer frames; null ⇒ GPU/BGRA-only.</summary>
    internal Func<IVideoCpuFrameConverter>? CpuConverterFactory => _cpuConverterFactory;
        ...
        VideoCompositorOptions? options = null)
    {
        ...
        return new VideoCompositor(output, compositor, options.CpuFrameConverterFactory);
    }
