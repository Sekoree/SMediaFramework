...
internal sealed partial class HaPlayPlaybackSession : IDisposable
{
    ...
    private HaPlayPlaybackSession(MediaPlayer player, PlaybackRouter router, OutputManagementViewModel outputs)
    {
        ...
        if (player.HasContainerDecoder && player.HasVideo)
        {
    ...
    private int SourceChannelCountOrFallback(int fallback = 2)
    {
        ...
        if (Player.HasContainerDecoder)
            return Player.AudioSource?.Format.Channels ?? fallback;
        return fallback;
    }
    ...
    private bool TryGetSourceAudioFormat(out AudioFormat format)
    {
        ...
        if (Player.HasContainerDecoder && Player.AudioSource is { } a && a.Format.IsValid)
        {
            format = a.Format;
            return true;
        }
        ...
        int? audioTrackIndex = null)
    {
        ...
        // Build the player from this host-managed decoder's sources (the framework no longer takes a concrete
        // decoder). videoOverride is the processed/negotiated video pipeline (null when audio-only). The cache
        // owns the decoder lifetime, so the player does not dispose the sources.
        var openDecoder = MediaPlayer.OpenLive(
                decoder.HasAudio ? decoder.Audio : null,
                videoOverride)
            .WithOptions(mpOpt)
    ...
    public void PrimeVideoOutputsBeforePlay(bool holdFallbackShowsImage = false, int frameCount = 12, int pacingMs = 2)
    {
        ...
        var hasVideo = IsLive ? LiveHasVideo : Player.HasVideo;
        if (!hasVideo)
        {
