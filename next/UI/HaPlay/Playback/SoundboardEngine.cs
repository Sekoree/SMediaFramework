...
public sealed class SoundboardEngine : IDisposable
{
    ...
    public async Task<string?> PlayAsync(SoundboardPlayRequest request)
    {
        ...
        try
        {
            ...
            {
                var builder = MediaPlayer.OpenFile(MediaRuntime.Registry, request.FilePath)
            ...
            if (player is null)
                return error;
            if (!player.HasContainerDecoder || !player.HasAudio)
            ...
            entry.SourceId = runtime.AddSource(
                player.AudioSource,
                BuildRouteSpecs(runtime, player.AudioSource.Format.Channels, request.Volume),
