...
public sealed partial class CuePlaybackEngine : IDisposable
{
        ...
        bool hasVideoPlacements)
    {
        ...
        var source = ClipMediaSource.FromBuilder(
            () => MediaPlayer.OpenFile(MediaRuntime.Registry, fileItem.Path)
    ...
    private void OnStandbyStatesChanged(IReadOnlyList<S.Media.Session.ClipPreparationStatus> states)
    {
    ...
    private async Task DisposeEntryAsync(ActiveCue entry, bool notifyEnded = true, TimeSpan releaseFade = default)
    {
        ...
        await Dispatcher.UIThread.InvokeAsync(() =>
        {
            // Detach subtitle overlays first (handle then source, in add order) so their feeds stop rendering
            // before the composition layers are torn down.
            foreach (var sub in entry.SubtitleOverlays)
            {
                try { sub.Dispose(); }
                catch (Exception ex) { Trace.LogWarning(ex, "CuePlaybackEngine: subtitle overlay dispose"); }
            }
        ...
        RoutePlan plan)
    {
        ...
        for (var i = 0; i < placements.Count; i++)
        {
            ...
            // Attach the cue's selected subtitle tracks once, onto the first composition it feeds. They render
            // as a top layer driven by the cue's media playhead (see AttachSubtitleOverlays).
            if (entry.SubtitleOverlays.Count == 0)
                AttachSubtitleOverlays(entry, runtime);
    ...
    /// <summary>
    /// Builds the cue's selected subtitle tracks into overlay sources and attaches them to the composition as
    /// top layers driven by the cue's media playhead. Embedded selections decode the chosen container stream;
    /// sidecar selections render the file directly. Deferred so probe/decode runs off the cue-start path.
    /// </summary>
    private void AttachSubtitleOverlays(ActiveCue entry, CueCompositionRuntime runtime)
    {
        var selections = entry.Cue.Subtitles;
        if (selections.Count == 0)
            return;
        if (entry.Cue.Source is not FilePlaylistItem fileItem)
            return;
        ...
        {
            var path = sel.IsEmbedded ? fileItem.Path : sel.Path;
            ...
            try
            {
                overlay = SubtitleOverlayFactory.FromFileDeferred(path, canvas.Width, canvas.Height, streamIndex, style);
            }
            catch (Exception ex)
            {
                Trace.LogWarning(ex, "CuePlaybackEngine: subtitle overlay build failed ({Path} stream {Stream})", path, streamIndex);
                continue;
            }
            ...
            try
            {
                var handle = runtime.AttachSubtitleOverlay(overlay, () => entry.Player.PlayClock.CurrentPosition);
                // Dispose order on teardown: detach the feed first, then the source.
                entry.SubtitleOverlays.Add(handle);
                entry.SubtitleOverlays.Add(overlay);
            }
            catch (Exception ex)
            {
                Trace.LogWarning(ex, "CuePlaybackEngine: subtitle overlay attach failed ({Path})", path);
                try { overlay.Dispose(); } catch { /* best effort */ }
            }
        }
    }
    ...
    private ClipAudioOutputRuntime? GetOrCreateAudioRuntime(Guid outputLineId)
    {
        ...
        try
        {
            ...
            // Register the clock with the genlock group for drift tracking. The ppm rate-slew actuation that
            // consumed a ratePpmProvider returns with the Phase-5 adaptive-rate output (ClipAudioOutputRuntime
            // no longer takes one); registration is kept for the OutputSyncGroup's drift measurement.
    ...
    private sealed class ActiveCue
    {
        ...
        /// <summary>Subtitle overlay handles + sources attached to this cue's composition (detach-handle then
        /// source, in add order). Empty unless the cue selected subtitle tracks.</summary>
        public List<IDisposable> SubtitleOverlays { get; } = new();
