...
internal sealed class PlaybackThroughputDiagnostics
{
    ...
    public void TryLogPeriodic(HaPlayPlaybackSession? session, IReadOnlyList<OutputLineViewModel> lines)
    {
        ...
        if (session.Player.HasContainerDecoder)
        {
            ...
            var audioPos = session.Player.HasAudio && session.Player.AudioSource is ISeekableSource a
                ? a.Position
                : (TimeSpan?)null;
            var videoPos = session.Player.HasVideo && session.Player.VideoSource is ISeekableSource v
                ? v.Position
                : (TimeSpan?)null;
